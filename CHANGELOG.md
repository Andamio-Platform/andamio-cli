...
- **`andamio course import-assignment <course-id> <module-code> <file.json>`** — publishes a quiz assignment (a `{"type": "quiz", "version": 1, …}` envelope, the format the Andamio app grades client-side) as the module's `assignment.content_json`, verbatim, sending only the `assignment` key. Until now this was a hand-built `curl` against the module-update endpoint with the JWT copied out of `~/.andamio/config.json` — the gap #62 closed for module creation, reopened for quizzes.
  ...
  The envelope is validated before any request with the same rules the FCB Fan Campus app enforces (fcb-fan-engagement-app `src/lib/quiz/quiz-envelope.ts`, pinned by fixtures), every violated rule is listed, and there is no bypass flag. The existing assignment's title, description, image and video URLs are preserved unless `--title` / `--description` override them. After the update the module is re-fetched and the stored value deep-compared to the file, so the command proves the opaque-`jsonb` assumption on the live gateway rather than trusting it. `--dry-run` prints the summary (question count, pass threshold, question ids, title source) and sends nothing; `--show-payload` adds the payload; `--output json` emits `{course_id, module_code, module_status, assignment: {title, title_source, question_count, pass_threshold, question_ids}, verified}`.
...
- **`course teacher publish-module` no longer warns "may not have been linked" on every publish.** The check read a `source` field the publish response never carries (it belongs to the merged read endpoints), so the warning — and its `register-module` hint — fired unconditionally, including for a module that came back `ON_CHAIN` with its `slt_hash` set in the same response. Linkage is now read from what the response actually says (`module_status` / `slt_hash`); the warning fires only for a module that is not linked. (#158)

- **`course create-module --approve --output json` now echoes the `slt_hash` it computed** (and `module_status: "APPROVED"`), so a caller no longer needs a follow-up `update-module-status` to learn the hash the command sent. Both fields are omitted without `--approve`. (#158)

- **Reads no longer fail on a degraded gateway (206 Partial Content).** `client.Get` accepted only 200, so the merged read endpoints' documented degraded answer — the normal `data` plus `meta.warning` when one backend is unavailable — surfaced as `API error 206: {…the data…}`, exit 1. `Get` now accepts the full 2xx range like every other verb. On a 206 the data is decoded normally and exits 0 (a partial result is not an error, any more than an empty one is); `meta.warning` stays in the `--output json` envelope for `get`-style commands and prints as `warning: …` on stderr in text mode. List commands keep their existing JSON shape unchanged on a 206 (a bare array — its shape must not depend on gateway health) and print the warning on stderr in every mode; the empty-list `{"data":[]}` envelope carries `meta`. Found on the 1.0 preprod dry-run. (#157)

- **`project manager qualified-contributors` reported zero contributors for every project.** The decoder used camelCase field tags (`projectId`, `totalCount`) and read the payload without unwrapping the gateway's `data` envelope, so every field silently stayed at its zero value — a project with fifty qualified contributors printed "No qualified contributors found." like one with none. Fixed to the real wire shape, pinned by a fixture. **`--output json` now passes the gateway payload through unchanged, in snake_case** (`project_id`, `aliases`, `total_count`, `truncated`, `status`) — the camelCase shape the 0.12.1 notes described was never actually emitted with data in it. The new `status` field explains an empty list (`ok` vs `no_prerequisites_configured`); text mode says which. (#90)

- **`course slts <course-id>` with no module code now says "module-code required"** instead of "course-id required" — the old message blamed the one argument that was supplied. (#157)
