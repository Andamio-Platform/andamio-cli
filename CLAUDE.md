...
| `course teacher register-module` | `/v2/course/teacher/course-module/register` | jwt | Register module from chain. `--course-id`, `--module-code`, `--slt-hash`. Idempotent on hash match: if the module already exists in DRAFT with a matching `slt_hash`, advances it to APPROVED; if already APPROVED/PENDING_TX/ON_CHAIN, exits as a no-op. **`--output json` envelope:** `{"action": "registered"\|"advanced"\|"already_registered", "status": "<current>", "slt_hash": "<supplied>", "advanced_from": "DRAFT"\|null, "response": <gateway-resp>\|null}`. Scripts that previously parsed gateway fields at the top level must read them under `.response.*`. |
