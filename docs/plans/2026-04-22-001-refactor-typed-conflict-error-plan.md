...
| Risk | Mitigation |
|------|------------|
| Gateway returns a non-409 status code for a module-code conflict (e.g., 400 or 422 from some future validation path). The new predicate's `errors.As` gate silently returns `false` and register-module loses its idempotency guarantee for that path. | **Delivered (todo #021):** verification against `andamio-api/internal/handlers/v2/merged_handlers/merged_handlers.go:881-892` confirmed the gateway returns **HTTP 400** (not 409) for `DUPLICATE_CODE` — the strict-only predicate *was* silently failing. `isModuleAlreadyExistsError` now layers a body-token fallback: if the error is not `*apierr.ConflictError` but `err.Error()` contains both `"already exists"` and `"course_module_code"`, treat as a conflict and emit a stderr warning about the status drift. Fixture: `internal/client/testdata/preprod-duplicate-module-response.md`. Follow-up to file against andamio-api to map the response to 409; when that lands the strict path fires first and the fallback's warning stops appearing. |
