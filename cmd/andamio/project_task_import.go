...
func runTaskImport(cmd *cobra.Command, args []string) error {
	...
	if !isJSON {
		...
		fmt.Printf("%s %d task files from %s/\n", action, len(mdFiles), taskDir)
	}
	...
	for _, existing := range existingTasks {
		if v, ok := existing["task_index"].(float64); ok && int(v) == index {
			...
			if status != "" && status != "DRAFT" {
				if !isJSON {
					fmt.Printf("  %s: SKIPPED (task %d is %s, not DRAFT)\n", filename, index, status)
				}
				return "skipped", nil
			}
		}
...
func updateTaskFromFile(c *client.Client, fm TaskFrontmatter, policyID, expirationMs string, contentJSON interface{}, index int, filename string, dryRun, isJSON bool) (string, error) {
	...
	if !isJSON && !dryRun {
		fmt.Printf("  %s: WARNING: updating existing task %d %q\n", filename, index, fm.Title)
	}

	if dryRun {
		data, _ := json.MarshalIndent(payload, "", "  ")
		if !isJSON {
			fmt.Printf("  %s: UPDATE task %d (dry-run)\n%s\n", filename, index, string(data))
		}
		return "updated", nil
	}
