...
### Security
- `internal/config.Save` now layers an explicit `os.Chmod(path, 0600)` after `os.WriteFile` so a previously-relaxed mode (e.g., `0644` from a manual `chmod`, a non-Unix backup restore, or a copy from a container layer) cannot persist across saves. `os.WriteFile` honors its mode argument only on file creation; without the chmod backstop, any saved secret — `UserJWT`, `DevJWT`, and especially the new 30-day `DevRefreshToken` — would inherit the existing relaxed permissions. Pre-existing latent bug, materially worse with PR-A's refresh-token addition; fix is one line. Regression test in `internal/config/config_test.go` (#80, surfaced during PR-A review). Atomic write-temp-then-rename is tracked as a follow-up in #84 to also close concurrent-writer races and the TOCTOU window during overwrite.
